...
public class ScrollAeronautics extends Scroll {
    public ScrollAeronautics() {
        super(
                NamespacedKey.fromString("aeronautics", plugin()),
                Scroll.FLAG_REGISTER_EVENTS,
                "Aeronautics",

                "Negates elytra crash-landing damage"
        );
    ...
    public void onEntityDamage(EntityDamageEvent event) {
        ...
        if (!damageType.equals(DamageType.FLY_INTO_WALL) && !damageType.equals(DamageType.FALL)) {
            return;
        }
        ItemStack chestArmorItemStack = player.getInventory().getItem(EquipmentSlot.CHEST);
        if (chestArmorItemStack == null || !chestArmorItemStack.getType().equals(Material.ELYTRA)) {
            return;
        }
