...
public class ScrollAqualensing extends Scroll {
    public ScrollAqualensing() {
        super(
                NamespacedKey.fromString("aqualensing", plugin()),
                Scroll.FLAG_SCHEDULE_PER_AFFECTED_PLAYER,
                "Aqualensing",

                "Grants night vision when underwater"
        );
        this.generators = new ScrollGenerator[] {
                new ScrollGeneratorTreasure(0.2, Chest.class, Structure.SHIPWRECK),
                new ScrollGeneratorMonsterDrop(0.01, EntityType.DROWNED)
        };
    }
    ...
    public void onScheduledTickPerAffectedPlayer(Player player) {
        ...
        if ((nightVisionEffect != null && nightVisionEffect.getDuration() > 220) || !Utils.isPlayerUnderwater(player)) {
            return;
        }
